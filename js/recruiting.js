...
async function api(path, { method = 'GET', body, silent } = {}) {
  const res = await fetch(CFG.apiBase + path, { method, headers: { 'Content-Type': 'application/json', ...(S.token ? { Authorization: `Bearer ${S.token}` } : {}) }, body: body ? JSON.stringify(body) : undefined });
  let json = {};
...
function connectSSE() {
  ...
  on('ROUND_STARTED', async d => { toast('Round open', `${d.round.name} — ${d.recruits} recruits are now available. Clocks are running.`, 'ok', 7000); await loadState(); draw(); });
...
async function draw() {
  ...
  else if (S.view === 'rankings') body = rankingsHTML();
...
function heroHTML() {
  ...
  const nextClose = openRecruits.length ? openRecruits.map(r => r.closesAt).sort()[0] : null;
  const team = myTeam() ? T(myTeam()) : null;
  const p = S.me?.points;
  const clock = cr?.status === 'PAUSED' ? `<div class="lbl">Recruiting</div><div class="time warn">PAUSED</div>`
    : nextClose ? `<div class="lbl">Recruiting closes in</div><div class="time" data-closes="${nextClose}" data-rid="0">${countdown(nextClose).text}</div>`
  ...
  return `<div id="rc-hero" class="rc-hero reveal in" style="--hero-a:${team ? team.colors.primary : '#6a0011'};--hero-b:${team ? team.colors.secondary : '#2a2f3a'}">
    ...
    <div class="rc-hero-inner">
      <div><div class="title">Recruitment<br>Center</div><div class="sub">${cr ? esc(cr.name) : (season ? esc(season.name) : 'No active recruiting season')}${st.season?.suspended ? ' · <span style="color:#ff5a6e">BIDDING SUSPENDED</span>' : ''}</div></div>
      <div class="rc-clock">${clock}${cr ? `<div class="round">${openRecruits.length} recruits open</div>` : ''}</div>
...
function visibleRecruits() {
  ...
  switch (f.status) {
    case 'available': list = list.filter(r => ['OPEN', 'PAUSED', 'UPCOMING'].includes(r.status)); break;
...
function boardHTML() {
  ...
  return `<div class="rc-board">
    <aside class="rc-side ${S.filtersCollapsed ? 'collapsed' : ''}">
      ...
      <div class="card rc-panel"><h4>Show</h4><div class="rc-pos">${[['available', 'Available'], ['all', 'All'], ['nobids', 'No bids'], ['signed', 'Signed'], ...(myTeam() ? [['leading', 'I lead'], ['outbid', 'Outbid'], ['mine', 'My bids'], ['watch', '★ Targets']] : [])].map(([k, l]) => `<button class="${f.status === k ? 'on' : ''}" data-rc="status:${k}">${l}</button>`).join('')}</div>
...
function cardHTML(r) {
  const me = myTeam(); const cls = r.status === 'SIGNED' ? 'signed' : (me && r.leader === me) ? 'lead' : (me && S.myBids?.has(r.id) && r.leader !== me && r.status === 'OPEN') ? 'outbid' : '';
  const canSelect = me && r.status === 'OPEN';
  return `<article class="card rc-card ${cls} ${S.selected.has(r.id) ? 'sel' : ''}" data-card="${r.id}">
    ...
    : `<div class="bidrow"><div class="cur">${r.currentBid ? `<img src="${logo(r.leader)}" alt=""><div><b>${r.currentBid}</b><small>${esc(T(r.leader).abbr)} leads</small></div>` : `<div><b class="nobid">—</b><small>No bids · min 1</small></div>`}</div>
        ${me && r.status === 'OPEN' ? `<button class="rc-btn ${r.leader === me ? 'dark' : 'primary'} sm" data-rc="bid:${r.id}">${r.leader === me ? 'Raise' : 'Place bid'}</button>` : `<button class="rc-btn sm" data-rc="open:${r.id}">Details</button>`}
...
async function openPlayer(id) {
  ...
  const canBid = me && r.status === 'OPEN' && !S.state.season?.suspended;
  ...
  const html = `<div class="rc-mhead"><div class="rc-avatar">${r.headshotUrl ? `<img src="${esc(r.headshotUrl)}" alt="">` : initials(r)}</div><div><h3>${esc(r.name)}</h3><div class="m"><span class="rc-stars-row s${r.stars}">${stars(r.stars)}</span><span>${r.position}</span><span>Overall #${r.rank}</span><span>${r.position} #${r.posRank}</span><span>${esc(S.state.rounds.find(x => x.id === r.roundId)?.name || '')}</span></div></div>
    ...
    <div class="rc-mbody"><div class="rc-mgrid"><div><h4>Ratings</h4>${attrsHTML(r, true)}<div class="rc-note" style="margin-top:8px">Overall ${r.overallActual} actual · ${r.overallPotential} potential · Recruit score ${r.score}</div>
      ...
      <div><h4>Current recruitment</h4><div class="rc-curbid">${r.status === 'SIGNED' ? `<img src="${logo(r.signedTeam)}" alt=""><div><b>${r.signedAmount}</b><small>Signed · ${esc(T(r.signedTeam).name)} · ${fmtDate(r.signedAt)}</small></div>` : r.currentBid ? `<img src="${logo(r.leader)}" alt=""><div><b>${r.currentBid}</b><small>${esc(T(r.leader).name)} leads${r.leader === me ? ' (you)' : ''}</small></div>` : `<div><b>—</b><small>No bids · minimum 1</small></div>`}</div>
        ${r.status === 'OPEN' ? `<div class="rc-note" style="margin-top:8px">Closes in <b data-closes="${r.closesAt}" data-rid="${r.id}">${countdown(r.closesAt).text}</b> · ${fmtDate(r.closesAt)}</div>` : `<div class="rc-note" style="margin-top:8px">Status: ${r.status}</div>`}
...
function rankingsHTML() {
  const pos = S.rankPos || 'ALL';
  const list = [...S.recruits.values()].filter(r => pos === 'ALL' || r.position === pos).sort((a, b) => a.rank - b.rank);
  return `<div class="rc-toolbar"><div class="rc-pos"><button class="${pos === 'ALL' ? 'on' : ''}" data-rc="rankpos:ALL">ALL</button>${POS_ORDER.filter(p => [...S.recruits.values()].some(r => r.position === p)).map(p => `<button class="${pos === p ? 'on' : ''}" data-rc="rankpos:${p}">${p}</button>`).join('')}</div><span class="grow"></span><span class="rc-note">Score = 40% actual + 60% potential (configurable by the commissioner)</span></div>
    <div class="card rc-tablewrap"><table class="rc-rank"><thead><tr><th>#</th><th>Pos #</th><th class="l">Player</th><th>Pos</th><th>★</th>${ATTRS.map(a => `<th title="${ATTR_NAMES[a]}">${a}</th>`).join('')}<th>OVR</th><th>POT</th><th>Score</th><th class="l">Status</th></tr></thead><tbody>${list.map(r => `<tr><td class="rk">${r.rank}</td><td>${r.posRank}</td><td class="l nm" data-rc="open:${r.id}" style="cursor:pointer;font-weight:600">${esc(r.name)}</td><td>${r.position}</td><td class="rc-stars-row s${r.stars}" style="letter-spacing:0">${r.stars}★</td>${r.act.map((v, i) => `<td class="attr" style="text-align:center;line-height:1.15"><b class="${rcls(v)}" style="display:block">${v}</b><span style="display:block;font-size:10px;color:var(--muted-2)">${r.pot[i]}</span></td>`).join('')}<td>${Math.round(r.overallActual)}</td><td>${Math.round(r.overallPotential)}</td><td><b>${r.score}</b></td><td class="l">${r.status === 'SIGNED' ? `<span class="rc-tag signed">${esc(T(r.signedTeam).abbr)} · ${r.signedAmount}</span>` : r.currentBid ? `${esc(T(r.leader).abbr)} ${r.currentBid}` : `<span class="rc-status ${r.status}">${r.status}</span>`}</td></tr>`).join('')}</tbody></table></div>`;
}
...
async function adminHTML() {
  ...
  <div class="rc-admin-grid">
    <div class="card"><div class="rc-livehd">Rounds · ${esc(s.name)}<span style="margin-left:auto;font-size:11px;color:var(--muted)">${k.total} recruits · ${k.signed} signed · ${k.unsigned} unsigned</span></div><ul class="rc-list rc-rounds">${A.rounds.map(r => `<li><div><b>${esc(r.name)}</b><div class="rc-note">${roundN(r.id)} players${r.startedAt ? ` · started ${fmtDate(r.startedAt)}` : ''}${r.pauseTotalMs ? ` · paused ${Math.round(r.pauseTotalMs / 60000)}m` : ''}</div></div><span class="rc-status ${r.status}">${r.status}</span><span class="acts">${['DRAFT', 'SCHEDULED'].includes(r.status) && !(cr && ['ACTIVE', 'PAUSED'].includes(cr.status)) ? `<button class="rc-btn sm primary" data-rc="startround:${r.id}">Start</button>` : ''}${['ACTIVE', 'PAUSED'].includes(r.status) ? `<button class="rc-btn sm" data-rc="closeround:${r.id}">Force close</button>` : ''}<button class="rc-btn sm ghost" data-rc="editround:${r.id}">Edit</button></span></li>`).join('')}</ul></div>
...
const ACTIONS = {
  ...
  editround: id => { const r = S.adminData.rounds.find(x => x.id === +id); modal(`<div class="rc-mhead"><div><h3>Edit round</h3></div><button class="x" data-rc="close">×</button></div><div class="rc-mbody"><div class="rc-form"><div class="full"><label>Name</label><input id="rc-r-name" value="${esc(r.name)}"></div><div><label>Window (hours, blank = season default ${S.state.season.windowHours})</label><input id="rc-r-hours" type="number" step="0.5" value="${r.windowHours ?? ''}"></div></div><div class="rc-mactions"><button class="rc-btn ghost" data-rc="close">Cancel</button><button class="rc-btn primary" data-rc="doeditround:${id}">Save</button></div></div>`, 'sm'); },
  doeditround: async id => { try { await api(`/admin/rounds/${id}`, { method: 'PATCH', body: { name: $('#rc-r-name').value, windowHours: $('#rc-r-hours').value === '' ? null : +$('#rc-r-hours').value } }); closeModal(); toast('✓ Round saved', '', 'ok'); await loadState(); draw(); } catch (e) { toast('Error', e.message, 'err'); } },
  settingsadmin: () => {},
  import: () => importWizard(),
  ...
  health: async () => { const h = await api('/admin/health'); const ok = v => v ? '<span class="rc-status ACTIVE">✓ OK</span>' : '<span class="rc-status LOCKED">✕ CHECK</span>'; modal(`<div class="rc-mhead"><div><h3>System health</h3></div><button class="x" data-rc="close">×</button></div><div class="rc-mbody"><table class="rc-adm"><tr><td>Database</td><td>${ok(h.database)}</td></tr><tr><td>Realtime connections</td><td>${h.realtime}</td></tr><tr><td>Settlement worker</td><td>${ok(h.settlementWorker)} <span class="rc-note">last run ${h.settlementWorker ? fmtTs(h.settlementWorker) : '—'}</span></td></tr><tr><td>Ledger reconciliation</td><td>${ok(h.ledger)} <span class="rc-note">${h.reconcile.issues.length} issue(s) · ${fmtDate(h.reconcile.at)}</span></td></tr></table>${h.reconcile.issues.length ? `<h4>Issues</h4><pre style="font-size:11px;white-space:pre-wrap">${esc(JSON.stringify(h.reconcile.issues, null, 1))}</pre>` : ''}<div class="rc-mactions"><button class="rc-btn ghost" data-rc="close">Close</button></div></div>`, 'sm'); },
  settingsadmin2: () => {},
};
ACTIONS.settings = () => { if (isCommish() && S.view === 'admin') { const s = S.state.season; modal(`<div class="rc-mhead"><div><h3>Recruiting settings</h3></div><button class="x" data-rc="close">×</button></div><div class="rc-mbody"><div class="rc-form"><div class="full"><label>Season name</label><input id="rc-set-name" value="${esc(s.name)}"></div><div><label>Recruit window (hours)</label><input id="rc-set-hours" type="number" step="0.5" value="${s.windowHours}"></div><div><label>Allow bid withdrawal</label><select id="rc-set-wd"><option value="0" ${!s.allowWithdrawal ? 'selected' : ''}>OFF (recommended)</option><option value="1" ${s.allowWithdrawal ? 'selected' : ''}>ON</option></select></div></div><div class="rc-mactions"><button class="rc-btn ghost" data-rc="close">Cancel</button><button class="rc-btn primary" data-rc="dosettings">Save settings</button></div></div>`, 'sm'); } else nav('settings'); };
ACTIONS.dosettings = async () => { try { await api('/admin/seasons/current', { method: 'PATCH', body: { name: $('#rc-set-name').value, windowHours: +$('#rc-set-hours').value, allowWithdrawal: $('#rc-set-wd').value === '1' } }); closeModal(); toast('✓ Recruiting settings saved', '', 'ok'); await loadState(); draw(); } catch (e) { toast('Not saved', e.message, 'err'); } };
